...
def file_log_content_splitter(log_file):
    # 获取脚本所在目录并创建 logs 目录
...
def file_log_content_grep(log_file_path, keyword):
    if keyword is None:
        print("Keyword is required for filtering.")
        return
    
    output_file_path = os.path.join('logs', '{}_filtered_log.log'.format(keyword))
