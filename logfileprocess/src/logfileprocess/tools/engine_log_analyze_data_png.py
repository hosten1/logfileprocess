...
def extract_and_plot_aslevels(log_file, batch_size=NATCH_SIZE*5):
    """
        ...
        if match:
            ...
            try:
                ...
                if 'ASLevels' in rtc_status_data:
                    ...
                    levels_values = [float(value) for value in levels_str.split('|')]
    ...
    if aslevels_data:
        ...
        # 创建子图结构
        fig, axs = plt.subplots(num_batches, 1, figsize=(IMAGE_WIDTH_PIXELS / DPI, (IMAGE_HEIGHT_PIXELS / DPI) * num_batches), dpi=DPI)
        ...
        for i in range(num_batches):
            ...
            # 绘制每个批次的数据
            axs[i].plot(aslevels_data[start_idx:end_idx], label='ASLevels', marker='o')
...
# *5 是因为数据是1s一次 其他的图是6s一次
def extract_and_plot_arlevels_recv(log_file, batch_size=NATCH_SIZE*5):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    logs_dir = os.path.join(script_dir, 'logs')
        ...
        if match:
            ...
            try:
                ...
                if 'ARLevels' in recv_status_data and 'peerID' in recv_status_data:
                    ...
                    arlevels_values = [float(value) for value in arlevels_str.split('|')]
    ...
    for peer_id, arlevels_data in peer_arlevels_data.items():
        print("All ARLevels for peerID {peer_id}: {arlevels_data}".format(peer_id=peer_id, arlevels_data=arlevels_data))
        ...
        if arlevels_data:
            ...
            for i in range(num_batches):
                start_idx = i * batch_size
                end_idx = start_idx + batch_size
                axs[i].plot(arlevels_data[start_idx:end_idx], label='ARLevels for peerID {}'.format(peer_id))
            ...
            plt.savefig(plot_path)
            plt.close()
            print("ARLevels plot for peerID {} saved as '{}'".format(peer_id, plot_path))

def extract_and_plot_levels_RS(log_file, batch_size=NATCH_SIZE):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    logs_dir = os.path.join(script_dir, 'logs')
            ...
            try:
                ...
                if 'ARLevels' in recv_status_data and 'peerID' in recv_status_data:
                    ...
                    if peer_id not in peer_arlevels_data:
                        peer_arlevels_data[peer_id] = []
                    peer_arlevels_data[peer_id].extend(arlevels_values)
                    print("Current ARLevels for peerID {peer_id}: {arlevels_values}".format(peer_id=peer_id, arlevels_values=arlevels_values))
            except ValueError:
                print("Error decoding JSON on line: {}".format(line))
    ...
    if aslevels_data:
        ...
        for i in range(num_batches):
            start_idx = i * batch_size
            end_idx = start_idx + batch_size
            axs[i].plot(aslevels_data[start_idx:end_idx], label='send audio level', color='blue')

            # 在每个子图中绘制每个 peer 的 ARLevels
            for peer_id, arlevels_data in peer_arlevels_data.items():
                if len(arlevels_data) > start_idx:
                    peer_arlevels_slice = arlevels_data[start_idx:end_idx]
                    axs[i].plot(peer_arlevels_slice, label='receive audio levels peer {}'.format(peer_id), linestyle='--')
...
def process_rtc_status_recv_data_logs(log_file_path, interval_s=6, batch_size=NATCH_SIZE):
    """
    ...
    """
    peer_data = defaultdict(lambda: {"recvBytes": [], "ARBitrate": [], "lossRate": []})
    timestamps = []
    ...
    try:
                ...
                if start_idx != -1 and end_idx != -1:
                    ...
                    if data.get("direction") == "recv" and "peerID" in data:
                        ...
                        if ar_packets + ar_packets_lost > 0:
                            loss_rate = ar_packets_lost / (ar_packets + ar_packets_lost)
                        else:
                            loss_rate = 0
        ...
        for peer_id, data in peer_data.items():
            # 确保数据点一致性
            num_points = min(len(data["recvBytes"]), len(data["ARBitrate"]), len(data["lossRate"]))
            ...
            for i in range(num_batches):
                ...
                axs[i].plot(timestamps_batch[start_idx:end_idx], data["recvBytes"][start_idx:end_idx], label="recvBytes (KB)", marker='o')
                axs[i].plot(timestamps_batch[start_idx:end_idx], data["ARBitrate"][start_idx:end_idx], label="ARBitrate (KB)", marker='s')
                axs[i].plot(timestamps_batch[start_idx:end_idx], data["lossRate"][start_idx:end_idx], label="Loss Rate", marker='^')

                axs[i].set_xlabel('Time (s)')
                axs[i].set_ylabel('Value')
                axs[i].set_title("PeerID: {} - Network Data (Batch {})".format(peer_id, i + 1))
                axs[i].legend()
                axs[i].grid(True)
            ...
            # 保存图片到 logs 目录
            output_file_path = os.path.join('logs', '{}_recv_other_data_{}.png'.format(log_file_name, peer_id))
...
def process_rtc_status_send_data_logs(log_file_path, interval_s=6, batch_size=NATCH_SIZE):
    """
    ...
    try:
                ...
                if 'rtc_status:' in line:
                    ...
                    if start_idx != -1 and end_idx != -1:
                        ...
                        if data.get("direction") == "send":
                            ...
                            send_bytes = data.get("sendBytes", 0) / 1000.0  # 转换成 KB
                            ...
                            if as_packets + as_packets_lost > 0:
                                loss_rate = as_packets_lost / (as_packets + as_packets_lost)
                            else:
                                loss_rate = 0
        ...
        # 确保数据点一致性
        num_points = min(len(send_data["sendBytes"]), len(send_data["lossRate"]), len(send_data["rtt"]))
        ...
        for i in range(num_batches):
            ...
            axs[i].plot(timestamps_batch[start_idx:end_idx], send_data["sendBytes"][start_idx:end_idx], label="sendBytes (KB)", marker='o')
            axs[i].plot(timestamps_batch[start_idx:end_idx], send_data["lossRate"][start_idx:end_idx], label="Loss Rate", marker='s')
            axs[i].plot(timestamps_batch[start_idx:end_idx], send_data["rtt"][start_idx:end_idx], label="RTT (ms)", marker='^')
        ...
        # 保存图片到 logs 目录
        output_file_path = os.path.join('logs', '{}_send_data.png'.format(log_file_name))
        plt.savefig(output_file_path)
        plt.close()
        print("图表已生成并保存到: {}".format(output_file_path))
