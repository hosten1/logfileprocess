...
class webrtc_log:
    # 函数：将日志文件拆分并保存
    def split_log_by_webrtc_init(log_file_path, output_dir='logs',callback=None):
        if callback is None:
        ...
        with open(log_file_path, 'r') as log_file:
            ...
            callback("Splitting webrtc log file: 开始查找包含\"WebRtcVoiceEngine::Init\" 行")
            # 统计匹配到的文件数量
            file_count = 0
            for line in log_file:
                ...
                if "WebRtcVoiceEngine::Init" in line:
                    ...
                    if timestamp_match:
                        ...
                        file_count += 1
                        callback("Splitting webrtc log file: 找到包含\"WebRtcVoiceEngine::Init\" 行，写入到:{}".format(current_filename))
            ...
            if current_file:
                current_file.close()
        callback("Splitting webrtc log file complete(have {} files)！！！".format(file_count))

    # 函数：从日志文件中提取 ".mm:" 行并保存到 objc 文件
    def extract_objc_lines_from_logs(output_dir='logs',callback=None):
        # 定义日期时间的正则表达式
                            ...
                            for line in log_file:
                                if '.mm:' in line:
                                    ...
                                    if date_pattern.match(line):
                                        is_mm_section = False  # 遇到新日期行，结束当前 .mm: 区段
                                    else:
                                        objc_file.write(line)  # 如果不是日期开头，继续写入
